...
color_palette = mb.cm.ocean(np.linspace(0, 1, 25))
...
from .network import MDNclustering


class corr2pt_estimator:
    def __init__(self, cat_path, ra_min=None, ra_max=None, dec_min=None, dec_max=None, mag_cut=None, theta= None, min_sep= 0.03, max_sep = 26, nedges=8, njk = 25,nangbins = 8, verbosity=2 ):
        ...
        self.theta = theta
        if theta is None:
            raise ValueError("Must provide the angular bins (theta)")
            
        self.xgrid = np.linspace(-.2,.5,10000)
        
        self.net = MDNclustering(5)
        ...
        if self.verbosity >1:
            print('The catalogue contains %s objects within a ra,dec range (%s,%s), (%s,%s)'%(len(dfin),np.round(dfin.ra_gal.min(),2), np.round(dfin.ra_gal.max(),2), np.round(dfin.dec_gal.min(),2), np.round(dfin.dec_gal.max(),2)))
            dfin_ = dfin.sample(10000)
            plt.scatter(dfin_.ra_gal, dfin_.dec_gal, s=2)
            plt.xlabel('RA')
            plt.ylabel('Dec')
            plt.show()
        
        
        ra_bins = np.linspace(dfin.ra_gal.min(),dfin.ra_gal.max(),int(np.sqrt(njk))+1)
        dfin['jk_ra'] = pd.cut(dfin['ra_gal'].values, ra_bins,labels=np.arange(0,5,1))

        dec_bins = np.linspace(dfin.dec_gal.min(),dfin.dec_gal.max(),int(np.sqrt(njk))+1)
        ...
        if self.verbosity >1:
            ...
            self.max_sep = self.max_sep# * self._units_to_degrees('arcmin')
            self.min_sep = self.min_sep# * self._units_to_degrees('arcmin')
            ...
            self.theta = 10**th * self._units_to_degrees('arcmin')
    ...
    def _compute_tree_distances_pairs(self, treeA, treeB, max_sep, theta ):
    ...
    def _compute_tree_distances(self, cutzA, cutzB, sep_edges=[0.03,26], i_jk=None, dtype='data'):
        ...
        if i_jk is not None:
            catALL = catd[catd.jk_index==i_jk]
        else:
            catALL = catd.copy()
            
        catA = catALL[(catALL.observed_redshift_gal>cutzA[0])&(catALL.observed_redshift_gal<cutzA[1])]
        catB = catALL[(catALL.observed_redshift_gal>cutzB[0])&(catALL.observed_redshift_gal<cutzB[1])]

        galsA_radec_kdt = np.c_[catA.ra_gal*np.cos(catA.dec_gal*np.pi/180), catA.dec_gal]
        galsB_radec_kdt = np.c_[catB.ra_gal*np.cos(catB.dec_gal*np.pi/180), catB.dec_gal]
        
        treeA = cKDTree(np.array([galsA_radec_kdt[:,0],galsA_radec_kdt[:,1]]).T)
        treeB = cKDTree(np.array([galsB_radec_kdt[:,0],galsB_radec_kdt[:,1]]).T)

        d = treeA.sparse_distance_matrix(treeB,
                            max_distance=sep_edges[1]*self._units_to_degrees('arcmin'),
                            output_type='ndarray')['v']
        
        if self.verbosity>2:
            plt.hist(d.flatten(), bins = 50, density=True, color ='deepskyblue', alpha =.3)
            plt.xlabel(r'$\theta$', fontsize=16)
            plt.ylabel(r'Frequency', fontsize=16)
            plt.show()
            
        if dtype=='data':
            self.angular_dist_data = d
        elif dtype=='randoms':
            self.angular_dist_randoms = d
        else: 
    ...
    def _jacknife_centers(self):
        ...
        for i in range(1,int(np.sqrt(self.njk)+1)):
            for j in range(1,int(np.sqrt(self.njk)+1)):        
                c = c+1
                df_jk = self.cat[(self.cat.jk_ra==i)&(self.cat.jk_dec==j)]
                cent_ra = 0.5*(df_jk.ra_gal.max()+df_jk.ra_gal.min())
                cent_dec = 0.5*(df_jk.dec_gal.max()+df_jk.dec_gal.min())
    ...
    def _create_randcat(self,nrand):
        ...
        df_rand = pd.DataFrame(np.c_[ra_rands,dec_rands], columns = ['ra_gal','dec_gal'])
        df_rand['jk_ra'] = pd.cut(df_rand['ra_gal'].values, np.linspace(self.cat.ra_gal.min(),self.cat.ra_gal.max(),int(np.sqrt(self.njk)+1)),labels=np.arange(0,int(np.sqrt(self.njk)),1))
        df_rand['jk_dec'] = pd.cut(df_rand['dec_gal'].values, np.linspace(self.cat.dec_gal.min(),self.cat.dec_gal.max(),int(np.sqrt(self.njk)+1)),labels=np.arange(0,int(np.sqrt(self.njk)),1))

        df_rand['jk_index'] = df_rand.jk_ra.values.astype(np.float) *int(np.sqrt(self.njk)) + df_rand.jk_dec.values.astype(np.float)  
        
        #temporary hack... 
        df_rand['observed_redshift_gal'] = np.random.uniform(0,1,len(df_rand))
    ...
    def _NN_trainingsample(self, size_training_point,Nsamps):
        out = torch.Tensor()
        inp = torch.Tensor()
        for jk in range(self.njk):
            djk = self._compute_tree_distances(cutzA = [0,6,0.7],cutzB = [0,6,0.7], sep_edges=[self.min_sep,self.max_sep], i_jk = jk)
                
            djk = pd.DataFrame(djk.flatten(), columns = ['d'])
            
            if len(djk) >1000000:
                djk=djk.sample(1000000)
                
            i = jk*torch.ones(size = (Nsamps,1))
            inp = torch.cat((inp,i),0)
            
            for b in range(Nsamps):
                out = torch.cat((out,torch.Tensor(djk.sample(1000).values)),1)
                
        out = out.T       
        data_training = TensorDataset(inp,out)
        loader = DataLoader(data_training, batch_size=100, shuffle = True)
        print('The training sample contains %s training points'%len(out))
        
        self.loader = loader
        return loader, out, inp, djk
    ...
    def _run_correlation(self,zrange1,zrange2, method = 'lz'):
        ...
        RA_ra, dec_ra, jk_ra, NRA_ = cat_rand.ra_gal.values, cat_rand.dec_gal.values, cat_rand.jk_index.values, len(cat_rand)
    ...
    def _network_training(self,epochs):
        optimizer = optim.Adam(self.net.parameters(), lr=1e-3) #, weight_decay=0.01)
        scheduler = lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.1)
        for epoch in range(epochs):
            for datain, xeval in self.loader:
                optimizer.zero_grad() 
                logalpha, mu, logsig= self.net(datain)
                sig = torch.exp(logsig)
                
                log_prob = logalpha[:,:,None] - logsig[:,:,None] - 0.5*((xeval[:,None] - mu[:,:,None])/sig[:,:,None])**2
    ...
    def _predict_distributions(self):
        ypreds_i = np.zeros(shape = (self.njk,10,len(self.xgrid)))
        for jk in range(self.njk):

            logalpha, mu, logsig= self.net(torch.Tensor([jk]).unsqueeze(0))
            ...
            for i in range(10):
                ypreds_i[jk,i] = alphas[0,i] * (1 / ((2*np.pi)**(1/2)*sig[0,i]) * np.exp(-0.5*((self.xgrid - mu[0,i])/sig[0,i])**2))
             
        ypreds_i = ypreds_i.sum(1) 
        ypreds_i = ypreds_i / ypreds_i.sum(1)[:,None]
        self.dangular_jk_pdf = ypreds_i
        return ypreds_i
    
    def _prob_from_pdf(self, prob):
        prob_jk = np.zeros(shape=(25,len(self.theta)-1))
        for i in range(len(self.theta)-1):
            xminus, xplus = np.argmin(np.abs(self.theta[i] -self.xgrid)), np.argmin(np.abs(self.theta[i+1]-self.xgrid))
            prob_jk[:,i] = prob[:,xminus:xplus].sum(1)
           
        self.prob_jk = prob_jk
        
        return prob_jk
    
    def _Dangular_randoms(self):
        pdf_rands = np.zeros(shape=(self.njk,len(self.theta)-1))
        for jk in range(self.njk):
            Dangular_rands = self._compute_tree_distances( cutzA=[0.6,0.7], cutzB=[0.6,0.7],i_jk=jk,dtype='randoms')
            Dangular_rands = pd.DataFrame(Dangular_rands, columns = ['d'])

            bin_edges = self.theta
            width = bin_edges[1:]-bin_edges[:-1]

            hist, _ = np.histogram(Dangular_rands.d.values, bins=bin_edges)
            pdf_rands[jk] = hist / np.sum(hist)
            
        self.pdf_rands = pdf_rands
        return pdf_rands
